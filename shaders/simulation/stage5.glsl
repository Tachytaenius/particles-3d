...
void computemain() {
	...
	for (uint boxId = 0; boxId < boxCount; boxId++) {
		...
		if (boxArrayData[boxId].start == invalidBoxArrayDatum) {
			// This box is empty
			continue;
		}
	...
	BoxArrayEntry arrayData = boxArrayData[particleBoxId];
	// Assume arrayData.start != invalidBoxArrayDatum
	for (uint i = arrayData.start; i < arrayData.start + arrayData.count; i++) {
		uint otherParticleId = sortedParticleBoxIds[i].particleId;
		// Assume sortedParticleBoxIds[i].boxId == particleBoxid
